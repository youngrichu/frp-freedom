...
def test_bypass_manager_ai():
    ...
    assert 'device_info' in analysis
    assert 'ai_analysis' in analysis
    assert 'security_assessment' in analysis['ai_analysis']
    assert 'bypass_strategy' in analysis['ai_analysis']
    assert 'vulnerability_score' in analysis['ai_analysis']
    assert 'recommended_methods' in analysis['ai_analysis']
    print(f"    - Analysis completed with {len(analysis['ai_analysis']['recommended_methods'])} recommendations")
    ...
    assert 'total_bypasses' in insights or 'message' in insights
    if 'total_bypasses' in insights:
        print(f"    - Total Bypasses: {insights['total_bypasses']}")
        print(f"    - Success Rates: {len(insights.get('success_rate_by_method', {}))} methods tracked")
        print(f"    - Trending Methods: {len(insights.get('trending_methods', []))} methods")
    else:
        print(f"    - Status: {insights['message']}")
